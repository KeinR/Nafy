...
class Font {
public:
    // The type that is recommended that the font
    // be passed around as, given that copying is
    // disabled.
...
private:
    ...
    FT_Face face;
    // 3-b, "From Memory" https://www.freetype.org/freetype2/docs/tutorial/step1.html#section-3
    // "Note that you must not deallocate the font file buffer before calling FT_Done_Face"
    // I take it this means that the face/font uses the raw font data, which makes sense.
    // That does mean however that it must keep a shared pointer to the data that it was generated from.
    ...
    // Construct a line with the given parameters
...
public:
    ...
    // Default initialize all to zero/nullptr
    // Calls to member functions will invoke undefined behavior
    Font();
